...
b = [0]*len(x)
b[0] = 3*(y[1]-y[0])/h[0]
b[len(x)-1] = 3*(y[len(y)-1]-y[len(y)-2])/h[len(x)-2]
...
for j in range(20):
    if (0.03 > (j / 20)) and (0.03 <= ((j + 1) / 20)):
        r = l[j][0] * 0.03 ** 3 + l[j][1] * 0.03 ** 2 + l[j][2] * 0.03 + l[j][3] - 1 / (1 + 25 * 0.03 ** 2)
        print('y*-y|0.03 = {}'.format(r))
    elif (0.97 > (j / 20)) and (0.97 <= ((j + 1) / 20)):
        r = l[j][0]*0.97**3 + l[j][1]*0.97**2 + l[j][2]*0.97 + l[j][3]-1/(1+25*0.97**2)
        print('y*-y|0.97 = {}'.format(r))
