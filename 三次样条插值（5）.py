...
a[0][0] = -1/(h[0]*h[0])
a[0][1] = 1/(h[1]*h[1])-1/(h[0]*h[0])
a[0][2] = 1/(h[1]*h[1])
a[len(x)-1][len(x)-3] = -1/(h[len(x)-3]*h[len(x)-3])
a[len(x)-1][len(x)-2] = 1/(h[len(x)-2]*h[len(x)-2])-1/(h[len(x)-3]*h[len(x)-3])
a[len(x)-1][len(x)-1] = 1/(h[len(x)-2]*h[len(x)-2])
for i in range(1, len(h)):
    a[i][i-1] = h[i]/(h[i-1]+h[i])
    a[i][i] = 2
    a[i][i+1] = h[i-1] / (h[i - 1] + h[i])
b = [0]*len(x)
b[0] = 2*((y[0]-y[1])/(h[0]*h[0]*h[0])+(y[2]-y[1])/(h[1]*h[1]*h[1]))
b[len(x)-1] = 2*((y[len(x)-3]-y[len(x)-2])/(h[len(x)-3]*h[len(x)-3]*h[len(x)-3])+(y[len(x)-1]-y[len(x)-2])/(h[len(x)-2]*h[len(x)-2]*h[len(x)-2]))
